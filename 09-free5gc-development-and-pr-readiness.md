...
- EES implementation branch `ees-only` at `8668372`
...
ees-only multi-AN test because upstream changed
...
For V0, the safer generic default is zero automatic Create retries.
...
though ees-only adds roughly 599 lines of handwritten logic.
...
Treat `ees-only` as the validated prototype and source of behavioral
